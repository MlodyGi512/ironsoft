...
private:
	...
	void mark_rest_result(const LoggerResult& result, const std::string& context_reason, std::int64_t now_ms);
	void update_link_health(std::int64_t now_ms);
	void refresh_rest_health(std::int64_t now_ms);
	void set_udp_link_alive(bool alive);
	void update_presence_state(std::int64_t now_ms, const std::string& reason_override = "");
	static std::int64_t mono_ms();
	static std::int64_t mono_ms(time_point tp);
	...
	std::int64_t last_rest_ok_ts_ms_ = 0;
